...
export interface AboutContentV2Props {
  lang: Lang;
}

export const AboutContentV2: FC<AboutContentV2Props> = ({ lang }) => {
  ...
  const journeyTitle = isFr ? "Parcours" : "Journey";
  const educationTitle = isFr ? "Formation" : "Education";
  ...
  const stackTitle = isFr ? "Stack technique" : "Tech stack";
  ...
  return (
    <>
      ...
      {/* Education */}
      <section class="mb-16">
        <h2 class="text-xs font-semibold uppercase tracking-widest text-[var(--color-muted)] mb-6">{educationTitle}</h2>
        <div class="space-y-4">
          {t.education.map((edu) => (
      ...
      {/* Stack */}
      <section class="mb-16">
        <h2 class="text-xs font-semibold uppercase tracking-widest text-[var(--color-muted)] mb-6">{stackTitle}</h2>
        <div class="flex flex-wrap gap-2">
          {["React", "Next.js", "TypeScript", "React Native", "Expo", "Tailwind CSS", "Node.js", "Convex", "Stripe", "Docker", "Jest", "Playwright", "Storybook", "GitHub Actions", "Sentry", "Vercel"].map((tech) => (
            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs bg-[var(--color-card)] border border-[var(--color-border)] rounded-full text-[var(--color-muted)] hover:text-[var(--color-text)] hover:border-[var(--color-accent)] transition-colors duration-200 cursor-default">
              {techIcons[tech] && <img src={techIcons[tech]} alt="" width="12" height="12" class="w-3 h-3 object-contain flex-shrink-0" />}
              {tech}
            </span>
          ))}
        </div>
      ...
      <section>
        ...
        <p class="text-[var(--color-muted)] mb-5">{contactText}</p>
